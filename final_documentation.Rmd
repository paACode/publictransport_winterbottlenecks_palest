...
As a next step, we simulate predictions using the GLM to estimate the likelihood of train delays, and then compare these simulated results with the actual dataset. We will have first a look at the mean of the simulated results:
...
This report set out to investigate the operational disruptions experienced by Zentralbahn in November 2024, with a focus on uncovering the root causes of winter-related performance bottlenecks and improving data reliability through predictive modeling. Through a structured exploration of various machine learning methods, the analysis identified critical patterns influencing both delay propagation and data integrity.

Initial modeling using artificial neural networks confirmed that the dataset contained meaningful signals, even without prior domain knowledge. This served as a foundation for more interpretable approaches. Generalized Additive Models demonstrated that while temperature and precipitation exhibit statistically significant relationships with delay metrics, their explanatory power remains limited. In contrast, operational and temporal factors—such as specific train lines or time of day have a more pronounced influence.

Generalized Linear Models, particularly in the binomial and Poisson frameworks, highlighted the importance of line-specific characteristics and contextual variables like rush-hour periods. Notably, the reopening of the Meiringen–Interlaken line emerged as a key inflection point, significantly altering delay dynamics. Additionally, Luzern’s function as a stabilizing hub was consistently associated with reduced delay propagation, underscoring the strategic importance of hub infrastructure.

Beyond performance modeling, the application of Support Vector Machines addressed a parallel objective: enhancing data quality. By formulating the recovery of missing station names as a classification problem, a methodological basis was established for scalable data imputation. Although performance was modest, the approach contributes to broader efforts to restore and maintain the integrity of historical transport records.

The findings underscore the multifactorial nature of railway delays, wherein environmental conditions act as secondary factors to more decisive operational and infrastructural elements. While current models offer valuable insights, a substantial portion of variability remains unexplained. This suggests the potential benefit of integrating additional data sources—such as real-time operational logs, staffing levels, or infrastructure status—into future modeling efforts.

In conclusion, the analysis not only confirms several hypotheses regarding winter bottlenecks but also provides a data-driven framework for identifying vulnerabilities and guiding operational decisions. By transforming raw transport and weather data into actionable insights, this work contributes to a more resilient and efficient transportation system of the Zentralbahn.
