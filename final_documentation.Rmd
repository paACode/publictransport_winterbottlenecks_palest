...
Understanding this relationship is crucial for the project's primary goal of identifying the root causes of performance bottlenecks during November 2024 and providing data-driven recommendations to improve punctuality and operational efficiency on affected lines for Zentralbahn.By quantifying the extent of delay propagation, this model provides insights into specific operational areas where buffer times may be insufficient or where interventions are needed to prevent delays from compounding across the network.

## Inital Model: Full Dataset

```{r model-buffer-effect, include=TRUE}
lm.fit_buffer <- lm(delay_departure ~ delay_arrival, data = d.trains)
summary(lm.fit_buffer)
```

```{r plot model-buffer-effect, include=TRUE}
...
```{r Filter for, echo=TRUE, include=TRUE}
d.trains.reduced <- d.trains %>% 
  filter(delay_arrival > 180)
  
```

```{r re-fit the linear model, include=TRUE}
...
## Conclusion
The Linear Model analysis, a key step in **identifying root causes of performance bottlenecks** for Zentralbahn during November 2024, revealed a **strong correlation between arrival and subsequent departure delays**. This indicates **minimal systematic buffer time** in the system, particularly when delays exceed approximately 3 minutes. Findings show that once a delay occurs, it tends to **propagate almost entirely** through the network. This suggests that while efficient, the system has **limited capacity to absorb larger disruptions** without impacting subsequent departures. These results are crucial for understanding the operational patterns contributing to bottlenecks and provide **data-driven insights** necessary for developing recommendations to improve punctuality and operational efficiency for Zentralbahn.
