...
**Categorical:** `BETRIEBSTAG`, `LINIEN_TEXT`, `HALTESTELLEN_NAME`, `START`, `ZIEL`, `DELAY_Category`, `TAGESZEIT`, `RUSH_HOUR`  
**Datetime:** `ANKUNFTSZEIT`, `AN_PROGNOSSE`, `ABFAHRTZEIT`, `AB_PROGNOSSE`  
**Continuous:** `ANKUNFTDELAY_sec`, `ABFAHRTDELAY_sec`, `w_precip_mm_Luzern`, `w_temp_min_c_Luzern`, `w_temp_avg_c_Luzern`  
**Binary:** `FAELLT_AUS_TF`
...
To explore whether the Zentralbahn railway system incorporates buffer times into its scheduling, we examine the relationship between **arrival delays** and subsequent **departure delays**.
...
Precipitation is also log-transformed due to its zero-inflated and skewed nature.
...
To better model this, we applied a log transformation to reduce skewness and improve fit.
While the transformation changes the scale, it enables a meaningful interpretation in terms of relative (percentage-based) changes.

From the `summary()` output of our baseline model:

-   **Slope (`log_arrival` ≈ 0.936)**: A 1% increase in arrival delay is associated with an approximate 0.94% increase in departure delay - indicating a strong, nearly proportional relationship.

-   **Intercept (\~0.105)**: After back-transforming, this corresponds to a **baseline multiplier of \~1.111**, meaning when arrival delay is 1 minute, expected departure delay is about **1.11 minutes**.

-   **R-squared (≈ 0.834)**: About 83% of the variation in log departure delay is explained by log arrival delay alone.

-   **Statistical significance**: The extremely low p-value for log_arrival (\< 2e-16) provides strong evidence that arrival delay affects departure delay.
    We can reject the hypothesis that arrival delay has no effect.
...
Next, we add external predictors – **log-transformed precipitation**, **temperature**, and **rush hour categories** – to see if delay propagation is influenced by weather or time of day.
...
-   `log_arrival` remains the dominant predictor, with a coefficient of approximately 0.936.
    This means that a **1% increase in arrival delay leads to an estimated 0.94% increase in departure delay** — a strong, nearly proportional relationship.
    The p-value (p \< 2e-16) provides **strong evidence** for this effect.

-   Rainfall and temperature have **no statistically significant effect**, as their p-values are far above conventional thresholds (p \> 0.3).
    We **fail to reject** the null hypothesis that they influence delay propagation.

-   There is **moderate evidence** that the **evening rush hour** slightly reduces delay propagation.
    The coefficient of -0.017 (p = 0.037) corresponds to an estimated **1.7% reduction in departure delay**, relative to non-rush hours.
    The morning peak shows no significant effect.

This suggests that delay propagation is robust across most conditions, but slightly more resilient during the evening.
...
-   `log_arrival` remains the **primary driver of departure delay**, with a coefficient of \~0.935.
    This means that a **1% increase in arrival delay corresponds to an estimated 0.94% increase in departure delay** — reaffirming the near-proportional delay propagation observed in earlier models.
    The effect is highly significant (p \< 2e-16).

-   Rainfall and temperature continue to show **no statistically significant** effect (p \> 0.3), and we **fail to reject** the hypothesis that they influence delay propagation.

-   The inclusion of station context provides new insight:

    -   Trains that **start in Luzern** show a coefficient of -0.118, which translates to a **\~11.2% lower departure delay** relative to other stations (exp(-0.118) ≈ 0.888).

    -   Trains that **end in Luzern** show a coefficient of -0.133, corresponding to a **\~12.5% reduction in delay** (exp(-0.133) ≈ 0.875).
...
Nonetheless, our models consistently show that **arrival delays are the dominant driver of departure delays**, with an estimated **elasticity of \~0.94**.
This implies that a **1% increase in arrival delay leads to a \~0.94% increase in departure delay**, indicating **near-complete delay propagation**.

Rainfall and temperature show **no statistically significant effect** on delay propagation.
In contrast, trains **starting or ending in Luzern** experience **\~11–13% lower departure delays**, suggesting that Luzern functions as a **stabilizing hub**, likely due to infrastructure or recovery capacity.

These insights suggest that operational improvements should focus on **non-hub stations**, where delay propagation is more pronounced.
Targeted increases in buffer time or recovery resources at these locations may yield the greatest reductions in network-wide disruptions.
...
```{r first-histogram, include=TRUE, echo=FALSE, fig.height=3}
...
```{r second-histogram, include=TRUE, echo=FALSE, fig.height=3}
...
```{r effect_temperature_depature_delay, include=TRUE, echo=FALSE, fig.width=5, fig.height=3}
...
```{r effect_percipitation_depature_delay, include=TRUE, echo=FALSE, fig.width=5, fig.height=3}
...
```{r first-plotting, include=TRUE, echo=FALSE, fig.width=5, fig.height=2.5}
#Plotting the data
...
```{r glm-delay, include=TRUE, echo=FALSE, fig.width=5, fig.height=2.5}
...
```{r boxplot-ANKUNFTDELAY_min-LINIEN_TEXT, include=TRUE, echo=FALSE, fig.width=8, fig.height=4}
...
We will have first a look at the mean of the simulated results:
...
mean(simulated_data$predicted_prob)
```
...
```{r Hist Skewed, include=TRUE, fig.height=3}
par(mfrow=c(1,3))
...
```{r Nr of Delay aggregated per Day and Operating Hour, include=TRUE, fig.height=3}
...
```{r Normalize for compariosn, include=TRUE, fig.height=3}
...
```{r Look at Days, include=TRUE, fig.height=3}
...
```{r Normalize for comparison 2, include=TRUE, fig.height=3}
# Count of observations per operation_category
category_counts <- d.poisson %>%
...
```{r Weather Data, include=TRUE, fig.height=3}
...
```{r Data for model, include=FALSE, fig.height=3}
...
```{r Model Validation, echo=FALSE, include= TRUE, fig.height=3}
...
In modern transportation systems, data incompleteness remains a persistent challenge.
Within Zentralbahn's operational datasets, one recurring issue is the absence of station names (HALTESTELLEN_NAME), which impairs downstream analysis and monitoring.
...
Specifically, we propose using Support Vector Machines (SVM), a well-established classification algorithm, to infer missing station names based on contextual features such as timestamps, weekday indicators, and route metadata.
This modeling task is framed as a multi-class classification problem, aiming to recover labels for incomplete entries.
...
::: {style="text-align: center;"}
```{r initial-analysis, echo=TRUE, include=TRUE, fig.width=5, fig.height=3}
...
The final dataset was prepared with balanced classes and scaled numerical features, enabling effective SVM training.
In the next section, we evaluate model performance using cross-validation to ensure robustness and generalizability.
...
::: {style="text-align: center;"}
```{r roc-curve, message=FALSE, warning=FALSE, include=TRUE, out.width="70%"}
...
To better understand the model's decision-making process, we analyzed feature importance based on the linear SVM’s learned weights.
The top five most influential variables are shown in the figure below.
Line identifiers such as `S5` and `IR` emerged as the strongest predictors, followed closely by directional routes involving Luzern (e.g., `Luzern_Giswil`, `Giswil_Luzern`).
...
While the linear kernel offered interpretability and computational efficiency, its performance was constrained by overlapping feature distributions and the inherent limitations of linear separability.
To address these constraints, we next explore a non-linear variant using the radial basis function (RBF) kernel.
...
::: {style="text-align: center;"}
```{r rbf-roc, include=TRUE, out.width="70%"}
...
To systematically compare the linear and radial SVM models, we summarized their performance across several classification metrics: AUC, precision, recall, and F1 score.
Both models were evaluated on the same dataset using the same cross-validation strategy to ensure a fair comparison.
While the radial SVM achieved slightly higher values across all metrics — including a marginal AUC increase from 0.629 to 0.644 — both models exhibited limited overall discriminative power.
The ROC curves show substantial overlap, with the radial model offering only a slight advantage in sensitivity at certain thresholds.
Given this modest performance gain and the radial model’s higher computational cost, the linear SVM remains the more practical choice for time-sensitive or large-scale applications.
...
::: {style="text-align: center;"}
```{r model-comparison 2, include=TRUE, out.width="70%"}
...
This report set out to investigate the operational disruptions experienced by Zentralbahn in November 2024, with a focus on uncovering the root causes of winter-related performance bottlenecks and improving data reliability through predictive modeling.
Through a structured exploration of various machine learning methods, the analysis identified critical patterns influencing both delay propagation and data integrity.

Initial modeling using artificial neural networks confirmed that the dataset contained meaningful signals, even without prior domain knowledge.
This served as a foundation for more interpretable approaches.
Generalized Additive Models demonstrated that while temperature and precipitation exhibit statistically significant relationships with delay metrics, their explanatory power remains limited.
In contrast, operational and temporal factors—such as specific train lines or time of day have a more pronounced influence.

Generalized Linear Models, particularly in the binomial and Poisson frameworks, highlighted the importance of line-specific characteristics and contextual variables like rush-hour periods.
Notably, the reopening of the Meiringen–Interlaken line emerged as a key inflection point, significantly altering delay dynamics.
Additionally, Luzern’s function as a stabilizing hub was consistently associated with reduced delay propagation, underscoring the strategic importance of hub infrastructure.

Beyond performance modeling, the application of Support Vector Machines addressed a parallel objective: enhancing data quality.
By formulating the recovery of missing station names as a classification problem, a methodological basis was established for scalable data imputation.
Although performance was modest, the approach contributes to broader efforts to restore and maintain the integrity of historical transport records.

The findings underscore the multifactorial nature of railway delays, wherein environmental conditions act as secondary factors to more decisive operational and infrastructural elements.
While current models offer valuable insights, a substantial portion of variability remains unexplained.
This suggests the potential benefit of integrating additional data sources—such as real-time operational logs, staffing levels, or infrastructure status—into future modeling efforts.

In conclusion, the analysis not only confirms several hypotheses regarding winter bottlenecks but also provides a data-driven framework for identifying vulnerabilities and guiding operational decisions.
By transforming raw transport and weather data into actionable insights, this work contributes to a more resilient and efficient transportation system of the Zentralbahn.
