...
## Finding reasonable Predictors "Weather

No clue yet how to interpret weather data
...
d.poisson.agg.day.weather <- d.poisson %>%
  ...
  summarize(
    ...
    daily_precipitation = first(w_temp_min_c_Luzern),
    .groups = "drop"
    ) %>% 
  ungroup()

plot(Count_Delayed ~ daily_min_temp ,  data = d.poisson.agg.day.weather)

plot(Count_Delayed ~ daily_precipitation ,  data = d.poisson.agg.day.weather)
