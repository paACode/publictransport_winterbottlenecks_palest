...
par(mfrow=c(1,2))
hist(d.delayed_trains$Count_Delayed, breaks = 20, main= "Original", xlab = "Count 'Delayed Trains '")
hist(log1p(d.delayed_trains$Count_Delayed), breaks=20, main = "Log Transformed ", xlab = "Count 'Delayed Trains'")
...
quantile_groups <- cut(d.delayed_trains$Count_Delayed, breaks=quantile(d.delayed_trains$Count_Delayed), 
                       include.lowest=TRUE, labels=c("Q1", "Q2", "Q3", "Q4"))

boxplot(d.delayed_trains$Count_Delayed ~quantile_groups)
...
```{r Overdispersion, include=TRUE}
ggplot(d.delayed_trains, aes(x =  operating_hour, 
                           y = Count_Delayed)) +
  geom_boxplot() +
   geom_jitter(width = 0.2, height = 0.1, color = "blue", alpha = 0.2)+
  labs(title = "Delayed Trains Count", 
...
```{r Operation Category, include=TRUE}
...
d.delayed.trains.simplified <- d.zentrahlbahn %>%
  filter(!is.na(operating_hour)) %>%  # Removes NA observations --> This are the Abfahrtszüge
  group_by(operation_category, BETRIEBSTAG) %>%
  summarize(Count_Delayed = sum(Delay_Category != "On Time"), .groups = "drop") %>% 
  ungroup()

ggplot(d.delayed.trains.simplified, aes(x = operation_category , 
                           y = Count_Delayed)) +
  geom_boxplot() +
   geom_jitter(width = 0.2, height = 0.01, color = "blue", alpha = 0.2)+
  labs(title = "Delayed Trains Count", 
...
library(multcomp)
d.delayed.trains.simplified$operation_category <- as.factor(d.delayed.trains.simplified$operation_category)
anova_model <- aov(Count_Delayed ~operation_category, data = d.delayed.trains.simplified)
